...
async function main() {
    ...
    const couponToOrderNums = parseExcelForOneToMany(
        filePath, 
        "Orders_With_Coupons_This_Year3",//"Sheet2", 
        COUPON_COLUMN, 
        ORDER_NUMBER_COLUMN, 
        { keyCaseOptions: makeKeysUpperCase } as ParseOneToManyOptions
    );
    const dealsMissingFromHubSpot: Record<string, string[]> = {};
    const totalNumberOfOrders = Object.values(couponToOrderNums)
        .reduce((acc, orderNumbers) => acc + orderNumbers.length, 0);
    log.info(`Total number of orders with coupons from sheet: ${totalNumberOfOrders}`);
    let entryIndex = 0;
    for (const [couponCode, orderNumbers] of Object.entries(couponToOrderNums)) {
        log.info(`Start of entryIndex: ${entryIndex+1}/${Object.entries(couponToOrderNums).length}`,
        ...
        for (const orderNum of orderNumbers) {
            ...
            if (
                ...
                log.warn(`searchResponse is undefined or No deal found for order number ${orderNum}.`);
            ...
            if (searchResponse.objectIds.length > 1) {
                log.warn(`Multiple deals found for order number ${orderNum}.`);
                STOP_RUNNING(1);
            }
        ...
        if (dealIdsToUpdate.length > 0) {
            ...
            log.warn(`No deals found to update for coupon code "${couponCode}".`);
        }
        entryIndex++;
        continue;
    }
